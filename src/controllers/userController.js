...
exports.atualizarPrecos = async (req, res) => {
    try {
      const { tipo } = req.body;
      const { porcentagemDesconto } = req.body;
  
      const produtos = await produtoModel.find({ tipo: tipo });
  
      produtos.forEach(async (produto) => {
        const precoAtual = produto.precoAtual;
        const novoPrecoPromocao = precoAtual - (precoAtual * porcentagemDesconto) / 100;
        
        await produtoModel.findByIdAndUpdate(produto._id, { precoPromocao: novoPrecoPromocao });
      });
